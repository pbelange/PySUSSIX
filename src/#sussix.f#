...
!  NSUS GE 1   ---> LOWER BOUND TO SUBTRACTS FREQUENCIES
!  NSUSN       ---> NUMBER OF FREQUENCIES TO BE SUBTRACTED
!  NSUSD       ---> DIMENSION: H=1, V=2, L=4
...
!       90 ascii input (filename: bpmXXXX)
...
!          Acknowledgement: Armando Bazzani has kindly agreed to
!          provide us with his tunenewt routine
...
!  MODIFIED:16/01/2005: Portability F77/F90, IO <=99
!  LAST MODIFIED: 02/06/2005: bpm files allowed 0001 to 9999
!
     ...
     &nline,nlst,nrc,nsus,nsusn,nsusd,nt1,nt2,ntot,nturn,ntwin,ntwix
      double precision eps,etune,tunex,tuney,tunez
      integer mterm
      parameter(mterm=900)
      integer mbpm
      double precision pieni,zero,one,two,pi,duepi,piph
      parameter(mbpm=10000,pieni=1d-17,zero=0d0,one=1d0,two=2d0)
...
!.....EVENTUALLY aux2 NTWIN IS AN OUTPUT PARAMETER OF READRIC!!
            if(ntwix.lt.ntwin) then
              ...
              if(nsus.ge.1.and.nsusn.gt.0) then
...
!.....N.B. NTURN IS AN OUTPUT PARAMETER OF DATSPE
     ...
     &-tunex,-tuney,-tunez,istune,etune)
              if(nsus.ge.1.and.nsusn.gt.0.and.(nsus+nsusn-1).le.mterm)  &
     &  then
!.....SUBTRACTS AND OVERWRITE UNIT IUNIT
                call susres(iunit,nsus,nsusn,nsusd,nturn,3)
              endif
            enddo
...
!............................................................
      ...
      subroutine susres(nfile,nsus,nsusn,nsusd,max,idams)
!=======================================================================
!
! SUBROUTINE SUSRES
!
...
! LEAVING THE TUNE IN.
!
! NSUS   = FIRST HARMONICS TO BE SUBTRACTED
! NSUSN  = LAST HARMONICS TO BE SUBTRACTED
! NSUSD  = DIMENSION: H=1, V=2, L=4
...
!=======================================================================
      implicit none
      integer idams,j,k,max,nfile,nsus,nsusn,nsusd
...
!....BUILD THE COMPLEX SIGNAL Z = X + i PX
      ...
      if(mod(nsusd,2).ne.0) then
        do j=nsus,nsus+nsusn-1
          zpotx=1d0
      ...
      if(nsusd.eq.2.or.nsusd.eq.3.or.nsusd.ge.6) then
        do j=nsus,nsus+nsusn-1
          zpoty=1d0
...
!  INPUT PARAMETERS:
!
!  NARM : NUMBER OF HARMONICS TO BE ORDERED
